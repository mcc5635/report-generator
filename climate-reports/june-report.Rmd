---
title: "Climate Risk Scenarios Report US-EMEA-ASIA"
author: "Michael Chase"
date: "06/25/2024"
monofont: Source Code Pro
output: 
  word_document:
    reference_doc: template.docx
---

## About the data

**econoomy.com**  (https://www.economy.com/) is a provider of economic analysis, data, forecasts, scenarios, models and advisory services.

```{r packages, include=FALSE, warning = FALSE, message = FALSE}
library(palmerpenguins)
library(dplyr)
library(ggplot2)
library(requests)
library(httr)
```

## Add API Call Here

# The EDF-X API provides easy access to these calculation methods and directly to PDs for the approximately 400 million (publicly listed and private) companies in Orbis globally




## Interesting Metrics

We can get some interesting mean metrics for each species:

```{r}
mean_metric <- penguins %>%
  group_by(species) %>%
  select(-year) %>%
  summarize(across(where(is.numeric), mean, na.rm = TRUE))
knitr::kable(mean_metric)
```

```{r, echo = FALSE, out.width='75%'}
# https://allisonhorst.github.io/palmerpenguins/reference/figures/carbon-markets.jpg
knitr::include_graphics("carbon-markets.jpg")
```

```{r, include = FALSE}
heaviest_specie <- mean_metric %>%
  filter(body_mass_g == max(body_mass_g)) %>% 
  pull(species)
```

::: {custom-style=highlight-box}
The **`r heaviest_specie`** specie is heavier than other ! 
:::

## Where do they live ? 

```{r species-fig, out.width='60%', echo = FALSE}
# https://allisonhorst.github.io/palmerpenguins/reference/figures/emission_types.png
knitr::include_graphics("emission_types.png")
```

```{r island-repart, echo = FALSE}
ggplot(penguins, aes(x = island, fill = species)) +
  geom_bar(alpha = 0.8) +
  scale_fill_manual(values = c("darkorange","purple","cyan4"),
                    guide = "none") +
  theme_minimal() +
  facet_wrap(~species, ncol = 1) +
  coord_flip()
```
